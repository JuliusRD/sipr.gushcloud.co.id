...
@section('title')
    Edit Reimbursement Request Detail | SIPR
@endsection
@section('header')
...
@section('content')
    <div class="row row-deck row-cards">
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="mb-0">{{$reimbursement->reimbursement_code ?? ''}}</h3>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-7">: {{$current_user->divisi->nama}}</div>
                                <div class="col-5">Request Date</div>
                                <div class="col-7">: {{$reimbursement->request_date}}</div>                           
                                <div class="col-5">Organization</div>
                                <div class="col-7">: {{$reimbursement->institusi->nama}}</div>
                                <div class="col-5">Leader</div>
                                <div class="col-7">: {{$reimbursement->leader->name}}</div>
                            </div>
                            <div class="row">
                                <div class="col-12">Notes (Reason of Request) :</div>
                                <div class="col-12">{{$reimbursement->note ?? '-'}}</div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="col-md-12 col-lg-9 d-block">
                ...
                <ul class="nav nav-tabs " data-bs-toggle="tabs">
                    <li class="nav-item">
                        <a href="#tabs-home-17" class="nav-link disabled" data-bs-toggle="tab">
                            <h3 class="card-title my-2">Step 1: Reimbursement Request</h3>
                        </a>
                    </li>
                    <li class="nav-item">
                            ...
                            <table class="table table-vcenter card-table">
                                ...
                                <tbody>
                                    @forelse ($reimbursement_detail as $key => $data)
                                        <tr>
                                            <td>{{ $reimbursement_detail->firstItem() + $key }}</td>
                                            <td>
                                                {{ $data->description ?? '' }}
                                            </td>
                                            ...
                                            <td>
                                                <a href="javascript:;" class="btn btn-info btn-sm editModalBtn"
                                                    data-id="{{ $data->id }}"
                                                    data-url="{{ route('reimbursement-detail.edit', $data->id) }}">Edit</a>
                                                <form class="form-delete"
                                                    onsubmit="return confirm('Konfirmasi hapus data ini?')"
                                                    action="{{ route('reimbursement-detail.destroy', $data->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <input type="hidden" name="url_back" value="{{ route('reimbursement.detail_edit', $reimbursement->id) }}">
            ...
            <div class="card mt-3">
                <div class="card-body">
                    <form method="POST" action="{{route('historyreimbursement.store')}}">
                        @csrf
                        <input type="hidden" name="reimbursement_id" value="{{$reimbursement->id}}">
                        <input type="hidden" name="status" value="memperbaharui">
...
<div class=" modal modal-blur fade" id="modal-report" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <form action="{{ route('reimbursement-detail.store') }}" method="POST">
            @csrf
